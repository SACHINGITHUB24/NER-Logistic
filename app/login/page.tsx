...
type Mode = 'official-login' | 'citizen-login' | 'citizen-register'

export default function LoginPage() {
  const [mode, setMode] = useState<Mode>('official-login')
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [name, setName] = useState('')
  const [loading, setLoading] = useState(false)
  ...
  const handleRegister = async () => {
    ...
    if (!res.ok) {
      ...
      if (signInRes?.error) {
        setError('Registered but sign-in failed. Please sign in manually.')
        setSuccess('')
  ...
  const isOfficial = mode === 'official-login'
  const isCitizenLogin = mode === 'citizen-login'
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 800, color: '#f1f5f9', fontSize: '0.9rem' }}>NER Logistics</span>
        <span style={{ color: '#475569', fontSize: '0.82rem' }}>← Back</span>
      </div>
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: '420px' }}>
          ...
          {/* Role toggle — top level */}
          <div style={{
            display: 'flex', background: '#0f172a', borderRadius: '12px',
            padding: '4px', marginBottom: '1.5rem', border: '1px solid #1e293b'
          }}>
            <button onClick={() => reset('official-login')} style={{
              flex: 1, padding: '10px', borderRadius: '9px', border: 'none',
              background: isOfficial ? 'linear-gradient(135deg, #0ea5e9, #0369a1)' : 'transparent',
              color: isOfficial ? '#fff' : '#64748b',
              fontSize: '0.85rem', fontWeight: isOfficial ? 700 : 400,
              cursor: 'pointer'
            }}>🛡️ Official</button>
            <button onClick={() => reset('citizen-login')} style={{
              flex: 1, padding: '10px', borderRadius: '9px', border: 'none',
              background: !isOfficial ? 'linear-gradient(135deg, #0ea5e9, #0369a1)' : 'transparent',
              color: !isOfficial ? '#fff' : '#64748b',
              fontSize: '0.85rem', fontWeight: !isOfficial ? 700 : 400,
          ...
          <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
            <div style={{ fontSize: '1.6rem', fontWeight: 900, color: '#f1f5f9', marginBottom: '6px' }}>
              {isOfficial ? 'Official Sign In' : isCitizenLogin ? 'Citizen Sign In' : 'Create Citizen Account'}
            </div>
            <div style={{ color: '#64748b', fontSize: '0.82rem' }}>
              {isOfficial
                ? 'Access the full NER decision dashboard'
                : isCitizenLogin
                ? 'Sign in to submit field reports'
                : 'Register to submit field reports from the ground'}
            </div>
          </div>

          {/* Demo credentials box — Official only
          {isOfficial && (
            <div style={{
              background: '#0ea5e915', border: '1px solid #0ea5e933',
              borderRadius: '10px', padding: '0.875rem', marginBottom: '1.25rem'
            }}>
              <div style={{ color: '#0ea5e9', fontSize: '0.75rem', fontWeight: 700, marginBottom: '6px' }}>
                🔑 Demo Credentials (SIH Evaluators)
              </div>
              <div style={{ color: '#64748b', fontSize: '0.78rem', lineHeight: 1.8 }}>
                Email: <span style={{ color: '#94a3b8', fontFamily: 'monospace' }}>official@ner.gov</span><br />
                Password: <span style={{ color: '#94a3b8', fontFamily: 'monospace' }}>test1234</span>
              </div>
              <button onClick={() => { setEmail('official@ner.gov'); setPassword('test1234') }} style={{
                marginTop: '8px', background: '#0ea5e922', border: '1px solid #0ea5e933',
                color: '#0ea5e9', borderRadius: '6px', padding: '5px 12px',
                fontSize: '0.75rem', cursor: 'pointer', fontWeight: 600
              }}>Auto-fill →</button>
            </div>
          )} */}

          {/* Citizen sub-tab: Login vs Register */}
          {!isOfficial && (
            <div style={{
              display: 'flex', background: '#0f172a', borderRadius: '10px',
              padding: '4px', marginBottom: '1.25rem', border: '1px solid #1e293b'
              ...
              <button onClick={() => reset('citizen-login')} style={{
                ...
                background: isCitizenLogin ? '#1e293b' : 'transparent',
                color: isCitizenLogin ? '#f1f5f9' : '#64748b',
                fontSize: '0.82rem', fontWeight: isCitizenLogin ? 600 : 400,
                cursor: 'pointer'
              }}>Sign In</button>
              <button onClick={() => reset('citizen-register')} style={{
                flex: 1, padding: '8px', borderRadius: '7px', border: 'none',
                background: isCitizenRegister ? '#1e293b' : 'transparent',
                color: isCitizenRegister ? '#f1f5f9' : '#64748b',
                fontSize: '0.82rem', fontWeight: isCitizenRegister ? 600 : 400,
                cursor: 'pointer'
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="••••••••" style={inputStyle}
                onKeyDown={e => e.key === 'Enter' && (isCitizenRegister ? handleRegister() : handleSignIn())} />
            </div>
            ...
            <button
              onClick={isCitizenRegister ? handleRegister : handleSignIn}
              disabled={loading || !email || !password}
              style={{
                width: '100%', padding: '12px',
                background: 'linear-gradient(135deg, #0ea5e9, #0369a1)',
                border: 'none', borderRadius: '8px', color: '#fff',
                fontSize: '0.95rem', fontWeight: 700, cursor: loading || !email || !password ? 'not-allowed' : 'pointer',
          ...
          {/* Official — no self-registration note */}
          {isOfficial && (
            <div style={{
              background: '#FF993311', border: '1px solid #FF993322',
              borderRadius: '10px', padding: '0.875rem', marginTop: '0.875rem',
              ...
              <div style={{ color: '#FF9933', fontSize: '0.75rem', fontWeight: 600, marginBottom: '3px' }}>
                🔒 Official accounts are pre-authorized
              </div>
              <div style={{ color: '#64748b', fontSize: '0.72rem' }}>
                Self-registration is not available for Official access. Contact your district administrator.
              </div>
            </div>
          )}
